...
BUZZER_PIN = 6       # GPB6 = 6 pin on port B
...
# Initialise MCP23017
mcp_write(0x01, 0x00)   # IODIRA = all outputs on Port A
mcp_write(0x13, 0x00)   # Turn all outputs off initially (GPIOB)
...
def play_tone(freq_hz=800, duration_sec=0.2):
    ...
    while time.time() < end_time:
        # Turn buzzer pin ON
        mcp_write(0x13, 1 << BUZZER_PIN)   # GPIOB
        time.sleep(half_period)
        # Turn buzzer pin OFF
        mcp_write(0x13, 0x00 << BUZZER_PIN)
        time.sleep(half_period)
