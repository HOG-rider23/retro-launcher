 ...
 *
 * REQUIRED: gpu_mem=64 (or higher) in /boot/firmware/config.txt
 *           The default gpu_mem=16 is too small for the vc4 KMS renderer.
 *
 * BUILD:
 *   g++ -std=c++17 -Wall -O2 -o chip8 chip8.cpp \
 *       $(pkg-config --cflags --libs sdl2)
 *
 * RUN (bare console, no desktop — the normal Pi Zero 2 W case):
 *   SDL_VIDEODRIVER=kmsdrm ./chip8 rom.ch8
 *
 * RUN (under X11/Wayland desktop, e.g. for testing on a PC):
 *   ./chip8 rom.ch8
 *
 * The binary auto-detects which backend to use, so you can just run:
 *   ./chip8 rom.ch8
 * and it will try kmsdrm first, then fall back to whatever SDL2 finds.
 *
 * ── Fixes summary ──────────────────────────────────────────────────────────
 *
 *  A. SDL2 backend: forces SDL_VIDEODRIVER=kmsdrm when no display env is set.
 *     Without this, SDL2 on a bare KMS console picks no backend and produces
 *     a black / corrupted screen.
 *
 *  B. Scale: SCALE=12 → window 768×384, wider than the 640-pixel panel.
 *     SCALE=10 → 640×320, letterboxed with 80 px black bars top and bottom.
 *
 *  C. Timing drift: lastCpu/lastTimer advance by the fixed period instead of
 *     being reset to clock::now(), so the emulator runs at a stable 700 Hz
 *     even when the Pi is briefly loaded.
 *
 * ── Key mapping ────────────────────────────────────────────────────────────
 *  CHIP-8   Keyboard
 *  1 2 3 C  →  1 2 3 4
...
static constexpr int SCALE     = 10;          // 640×320 game area
...
// Pixel colours
static constexpr uint8_t FG_R=255, FG_G=255, FG_B=255; // on  = white
static constexpr uint8_t BG_R=  0, BG_G=  0, BG_B=  0; // off = black
...
struct Renderer {
    ...
    // Destination rect: game area centred in the 640×480 screen
    ...
    void init(const char* title) {
        ...
        // Use the accelerated renderer (vc4 via KMS/DRM).
        // Do NOT use SDL_RENDERER_SOFTWARE here — on a bare KMS console the
        // software renderer writes to a shadow buffer that never reaches the
        // display.  The hardware renderer goes through the DRM page-flip path.
        renderer = SDL_CreateRenderer(window, -1,
            SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
        if (!renderer) {
            // Fallback without vsync (e.g. kmsdrm without vsync support)
            renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);
        }
        if (!renderer) throw std::runtime_error(
            std::string("SDL_CreateRenderer: ") + SDL_GetError());

        // ARGB8888 — the vc4 KMS driver colour-manages the DPI channel order
        // internally, so userspace always sees a standard ARGB surface.
...
// ─── Backend selection ────────────────────────────────────────────────────────
//
// On a bare KMS console (no X11/Wayland), SDL2 must use the "kmsdrm" driver.
// If SDL_VIDEODRIVER is already set in the environment we honour it.
// Otherwise we try kmsdrm first (correct for Pi Zero 2 W + DPI display),
// then fall back to whatever SDL2 auto-detects (works on a desktop).

static bool initSDL() {
    // If the user already set SDL_VIDEODRIVER, use it directly.
    if (SDL_getenv("SDL_VIDEODRIVER")) {
        return SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO) == 0;
    }

    // Try kmsdrm (bare console / Pi DPI display)
    SDL_setenv("SDL_VIDEODRIVER", "kmsdrm", 1);
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO) == 0) return true;
    std::cerr << "[init] kmsdrm failed (" << SDL_GetError()
              << "), trying auto-detect\n";

    // Fall back: clear the hint and let SDL2 pick
